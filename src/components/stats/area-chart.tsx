          ...
          {payload.map(({ value, category, color }, index) => (
            <div key={`id-${index}`} className='flex items-center justify-between space-x-8'>
              <div className='flex items-center space-x-2'>
                <span
                  aria-hidden='true'
                  className={cn('h-[3px] w-3.5 shrink-0 rounded-full', getColorClassName(color, 'bg'))}
                />
                <p
                  className={cn(
                    // base
            ...
            return (
              <React.Fragment key={category}>
                <defs key={category}>
                  <linearGradient
                    key={category}
                    className={cn(getColorClassName(categoryColors.get(category)!, 'text'))}
                    id={categoryId}
                    x1='0'
                    y1='0'
                    x2='0'
                ...
                <Area
                  className={cn(getColorClassName(categoryColors.get(category)!, 'stroke'))}
